...
def get_arguments(x, z, 
    ...
    for ai in range(arguments.shape[1]):
        ...
        def _convert_to_blob_(argument):
            argument = cv2.resize(argument, tuple(size), \
                            interpolation = cv2.INTER_CUBIC)
            argument = (argument - argument.min())/(argument.max() - argument.min() + 1e-5)
            binary_mask = 255.0*(argument > 0.8)
            contours, hierarchy = cv2.findContours(np.uint8(binary_mask), 
                                                        cv2.RETR_LIST, 
                                                        cv2.CHAIN_APPROX_NONE)

            return_argument = np.zeros_like(binary_mask)
        ...
        Bmask = np.array([_convert_to_blob_(Fd) for Fd in Bmask])
    ...
    return_arguments = np.swapaxes(return_arguments, 0, 1) # bs x nargs
    argument_idx = np.swapaxes(np.array(argument_idx), 0, 1) # bs x nargs
    return  return_arguments, argument_idx
...
def main_image(args, epoch, plot=False):
    ...
    if plot:
        ...
        argument_dictionary = {}
        supporting_agent = 0
        supporting_arguments = logs_data['arguments_idx'][supporting_agent]
        for _argument_ in np.unique(supporting_arguments):
            if _argument_ not in argument_dictionary.keys():
                argument_dictionary[_argument_] = []

            img_idx, arg_idx = np.where(supporting_arguments == _argument_)

            for ibatch in np.unique(img_idx):
                img = normalize(logs_data['images'][ibatch])
                mask = logs_data['visual_arguments'][supporting_agent][ibatch][np.where(supporting_arguments[ibatch] == _argument_)[0][0]]
                
                blend_mask = np.zeros_like(img)
                blend_mask[mask[:, :, 0] == 0] = (0, 0, 0)
                blend_mask[mask[:, :, 0] > 0] = (255, 255, 255)
                dst = cv2.addWeighted(img, 0.5, blend_mask, 0.5, 0.0)
                argument_dictionary[_argument_].append(dst)
        ...
        ncols = 2*narguments + args.nagents
        ...
        for imgidx in range(0, nrows, args.nagents):
            title = '$Y={}$, $\mathcal{{J}}(x)={}$, $\Gamma(x)={}$, '.format(logs_data['labels'][imgidx//2],
                                                                        logs_data['jpred'][imgidx//2],
                                                                        logs_data['outcome'][imgidx//2])
            # if logs_data['labels'][imgidx//2] != logs_data['jpred'][imgidx//2]: continue

            if logs_data['outcome'][imgidx//2] != logs_data['jpred'][imgidx//2]: continue
            if logs_data['predictions'][0][imgidx//2] == logs_data['predictions'][1][imgidx//2]: continue
            if logs_data['predictions'][supporting_agent][imgidx//2] != logs_data['outcome'][imgidx//2]: continue

            for aidx in range(args.nagents):
                title += '$\Sigma^{} = {}$, '.format(aidx+1, 
                                                logs_data['predictions'][aidx][imgidx//2])
                                    
                for argidx in range(ncols - narguments):
                    if argidx == 0:
                        ...
                        input_img = normalize(logs_data['images'][imgidx//2])
                        mask = logs_data['visual_arguments'][aidx][imgidx//2, argidx - args.nagents]
                        ...
                        # ===============================================
                        ax2 = plt.subplot2grid(shape=(nrows, ncols), 
                                                loc=(imgidx + aidx, narguments + argidx))
                        supporting_argument_idx = logs_data['arguments_idx'][1 - supporting_agent][imgidx//2, argidx - args.nagents]
                        if supporting_argument_idx not in argument_dictionary: continue
                        example_index = np.random.choice(len(argument_dictionary[supporting_argument_idx]))
                        supporting_argument = argument_dictionary[supporting_argument_idx][example_index]

                        ax2.imshow(supporting_argument)
                        ax2.get_xaxis().set_visible(False)
                        ax2.get_yaxis().set_visible(False)
                        ax2.set_title('CE: $\mathcal{{A}}^{}_{} = {}$'.format(supporting_agent + 1, 
                                                                argidx - args.nagents + 1,
                                                                supporting_argument_idx))

                        # print((nrows, ncols),  (imgidx + aidx, argidx), (imgidx + aidx, narguments + argidx))
...
if __name__ == "__main__":
    ...
    name = args.name
    os.makedirs(os.path.join(args.plot_dir, 'Testpngs'), exist_ok=True)
